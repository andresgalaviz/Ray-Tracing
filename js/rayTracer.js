...
// Switch between light rotation and sphere rotation
function lightMode( el ) {
	if ( el.value === "Rotate lights" ) {
		operationMode = 1;
		el.value = "Rotate spheres";
	} else {
		operationMode = 0;
		el.value = "Rotate lights";
	}
}

var gpu = new GPU();

// Obtain the magnitude of the vector defined by the x, y, z components
function MagnitudeVector(x,y,z) {
	return Math.sqrt(Math.pow(x, 2) + Math.pow(y,2) + Math.pow(z,2));
}

// Preprocessing to obtain the scene parameters
...
// Preprocessing to obtain the coordinate system of the camera
...
function sphereIntersection(rayXVector, rayYVector, rayZVector, rayXPoint, rayYPoint, rayZPoint, sphereXPoint, sphereYPoint, sphereZPoint, sphereRad) {
    ...
    // Dotproduct of the vector from the camera point to the sphere center
...
function renderLoop() {
	...
	if(operationMode == 0) {
		...
		light2 += 0.05; 
		light3 += 0.05; 
		lights[1] = -100 + Math.sin(light1) * 35;
		lights[2]= -50 + (Math.cos(light1) * 35);

		lights[7] = 50 + Math.sin(light2) * 40;
		lights[8] = 100 + (Math.cos(light2) * 40);


		lights[13] = -100 + (Math.sin(light3) * 40);
		lights[14] = -150 + (Math.cos(light3) * 40);
	}
